...
def add_color(tweets):
    ...
    for t in tweets:
        color_index = round((t['score']+1)*50)
...
def tweets(name):
    ...
    rdict = fetch_tweets(api, name)
    tweets = rdict['tweets']
    scores = [t['score'] for t in tweets]
    med = median(scores)
    tweets = add_color(tweets)
    return render_template('tweets.html', name=name, med=med, tweets=tweets)
...
def following(name):
    ...
    """
    rlist = fetch_following(api, name)
    rlist = sorted(rlist, key=lambda rlist: rlist['followers'], reverse=True)
    return render_template('following.html', name=name, rlist=rlist)
...
#twitter_auth_filename = '/Users/minchenwang/classes/692/sentiment-minchenw/twitter.csv'
#api = authenticate(twitter_auth_filename)
